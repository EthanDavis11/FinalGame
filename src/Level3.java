...
public class Level3 extends BasicGameState {
    ...
    //
    Background=new Image("Images/Free/Background/Gray.png");
    ...
    Garry = new Enemy(100,50,10,1,400,500,iE,rrE,rlE,jE,fE,HE,die,false,false,true,true);
    
    George = new Enemy(100,50,10,1,300,500,iE,rrE,rlE,jE,fE,HE,die,false,false,true,true);
    ...
    public void update(GameContainer gc, StateBasedGame sbg, int i) throws SlickException { 
        ...
        //Input in = gc.getInput();
            ...
            if(ground.getY()<=Garry.getYP()+32){
                Garry.setGround(true);
            }
            else{
               Garry.setGround(false);
            }
            
            if(ground.getY()<=Garry.getYP()+32){
                George.setGround(true);
            }
            else{
               George.setGround(false);
            }
            
            if(Bob.seePlayer(Franklen)==true) Bob.MoveToPlayer(Franklen);
            
            if(Garry.seePlayer(Franklen)==true) Garry.MoveToPlayer(Franklen);
            
            if(George.seePlayer(Franklen)==true) George.MoveToPlayer(Franklen);
            ...
            if(Garry.getHitBox().intersects(Franklen.getHitBox())&&Garry.getHitBox().getX()<Franklen.getHitBox().getX()&&Garry.getHitBox().getY()==Franklen.getHitBox().getY()){
                Franklen.hit();
            }
            
            else if(Garry.getHitBox().intersects(Franklen.getHitBox())&&Garry.getHitBox().getX()>Franklen.getHitBox().getX()&&Garry.getHitBox().getY()==Franklen.getHitBox().getY()){
                
                Franklen.hit2();
            }
            
            if(Garry.getHitBox().intersects(Franklen.getHitBox())&&Garry.getHitBox().getY()>Franklen.getHitBox().getY()){
                Garry.setHealth(0);
            }
            
            if(George.getHitBox().intersects(Franklen.getHitBox())&&George.getHitBox().getX()<Franklen.getHitBox().getX()&&George.getHitBox().getY()==Franklen.getHitBox().getY()){
                
                Franklen.hit();
            }
            
            else if(George.getHitBox().intersects(Franklen.getHitBox())&&George.getHitBox().getX()>Franklen.getHitBox().getX()&&George.getHitBox().getY()==Franklen.getHitBox().getY()){
                
                Franklen.hit2();
            }
            
            if(George.getHitBox().intersects(Franklen.getHitBox())&&George.getHitBox().getY()>Franklen.getHitBox().getY()){
                George.setHealth(0);
            }
            
            if(finish.intersects(Franklen.getHitBox())&&Bob.getXP()==50000&&Garry.getXP()==50000&&George.getXP()==50000){
    ...
    g.drawString("Elimate Bob, Garry and George by Jumping on their heads to win", 375, 10);
    ...
    if(Garry.getHealth()>0)
        Garry.getAnimation().draw(Garry.getXP(),Garry.getYP());
    
    if(George.getHealth()>0)
        George.getAnimation().draw(George.getXP(),George.getYP());
